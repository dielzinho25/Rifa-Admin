(function(){
'use strict';
let reservasCache={}, numerosCache={}, usuarioAtual=null, ouvindo=false, primeiraLeituraReservas=true, resumoAnterior={total:0,pagamentos:0};
...
const secoes=['barraAdmin','perfilAdmin','dashboard','novosAdmins','configPagamento','configPix','sorteioAdmin','painel'];
...
function ouvirPendentes(){db.ref('rifa/administradores').orderByChild('status').equalTo('pendente').on('value',s=>{const c=$('listaAdminsPendentes');c.innerHTML='';const dados=s.val()||{};const lista=Object.values(dados);if(!lista.length){c.innerHTML='<p class="texto-apoio">Nenhum cadastro aguardando aprovação.</p>';return;}lista.forEach(p=>{const d=document.createElement('article');d.className='admin-pendente';d.innerHTML='<strong>'+esc(p.nome||'Sem nome')+'</strong><p>'+esc(p.email||'')+'</p><p>'+esc(p.telefone||'')+'</p><div class="linha-botoes"></div>';const a=d.querySelector('.linha-botoes');const ok=document.createElement('button');ok.className='botao confirmar';ok.textContent='✓ Aprovar';ok.onclick=()=>aprovarAdmin(p.uid,true);const no=document.createElement('button');no.className='botao excluir';no.textContent='Recusar';no.onclick=()=>aprovarAdmin(p.uid,false);a.append(ok,no);c.appendChild(d);});});}
...
async function ativarNotificacoes(){if(!('Notification'in window))return alert('Este navegador não suporta notificações.');const p=await Notification.requestPermission();$('btnNotificacoes').textContent=p==='granted'?'🔔 Avisos ativados':'🔕 Avisos bloqueados';if(p==='granted')new Notification('Painel da rifa',{body:'Notificações ativadas com sucesso.'});}
function avisar(titulo,corpo){if(document.hidden&&'Notification'in window&&Notification.permission==='granted')new Notification(titulo,{body:corpo,icon:'icon-192.svg'});}
function desenharLinks(m){const l=$('listaLinks');l.innerHTML='';const qs=Object.keys(m).sort((a,b)=>a-b);if(!qs.length){l.innerHTML='<p class="texto-apoio">Nenhum link configurado.</p>';return;}qs.forEach(q=>{const d=document.createElement('div');d.className='link-item';d.innerHTML='<div><strong>'+q+' número(s) — '+moeda(Number(q)*20)+'</strong><small>'+esc(m[q])+'</small></div>';const b=document.createElement('button');b.className='botao cancelar';b.textContent='🔒 Excluir';b.onclick=()=>{if(proteger('Excluir este link?'))db.ref('rifa/configPublica/pagamentos/links/'+q).remove();};d.appendChild(b);l.appendChild(d);});}
async function salvarLink(){const q=Number($('quantidadeLink').value),u=$('urlLink').value.trim();if(!Number.isInteger(q)||q<1||q>100)return alert('Quantidade inválida.');try{const url=new URL(u);if(url.protocol!=='https:')throw 0;}catch(e){return alert('Link HTTPS inválido.');}await db.ref('rifa/configPublica/pagamentos/links/'+q).set(u);$('quantidadeLink').value='';$('urlLink').value='';}
...
function atualizarDashboard(){
  ...
  const livres=Math.max(0,100-reservados-confirmados);
...
async function baixarBackup(){
  try{
    ...
    const pacote={versao:13,criadoEm:Date.now(),projeto:'rifa-firebase',rifa:s.val()||{}};
